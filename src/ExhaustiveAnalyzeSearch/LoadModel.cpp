...
static HMODULE G_This_LoadDll_Module = NULL;
static WCHAR   G_This_LoadDll_Name[MAX_PATH];
...
DWORD WINAPI MyGetModuleFileNameA(HMODULE hModule, LPSTR lpFilename, DWORD nSize)
{
    ...
    {
        return WideCharToMultiByte(CP_ACP, 0, G_This_LoadDll_Name, -1, lpFilename, nSize, NULL, NULL);
    }
    PGetModuleFileNameA realfunc = (PGetModuleFileNameA)GetProcAddress(LoadLibraryA("Kernel32.dll"), "GetModuleFileNameA");
        ...
        if (impDllModule)
        {
            for (int i=0; ImportApiName[i]!=NULL ; i++)
            {
                __try
                {
                    ...
                    {
                        FixTable[i] = (LPDWORD)GetProcAddress(impDllModule, (PSTR)(ImportApiName[i]&0x7FFFFFFF));
                    }
                    else
                    {
...
BOOL CopyModuleSection(HMODULE SrcHmod, HMODULE DstHmod)
{
    ...
    for (DWORD i=0; i<SectionNum; i++) // 将节一个个复制到内存中
    {
            ...
            )
        {
            ...
            memcpy( (LPBYTE)DstHmod + SectionHeader[i].VirtualAddress,
                (LPBYTE)SrcHmod + SectionHeader[i].VirtualAddress,
                ulsize);
        }
    }
    return true;
...
HMODULE WINAPI LoadDllFromMemory(LPVOID pMemAddr, BOOL bCallDllMain)
{
...
// 加载DLL函数
HMODULE WINAPI LoadDllFromFileW(PWSTR szFileName, BOOL bCallDllMain)
{
    ...
    hModule = LoadDllFromMemory(pFile, bCallDllMain);
...
HMODULE WINAPI LoadDllFromFileA(PSTR szFileName, BOOL bCallDllMain)
{
    LPWSTR lpwzFileName = MulToWide(szFileName);
    HMODULE hModule = LoadDllFromFileW(lpwzFileName, bCallDllMain);
